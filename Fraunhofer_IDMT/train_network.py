...
def build_datasets(train_percentage=0.8, preproc=False):
    if (preproc):
        ...
        path = "/home/karim/UrbanSound8K/AudioClassification_Deep_Learning/Fraunhofer_IDMT/Samples/"
...
def build_model(X,Y,nb_classes):
    ...
    model = Sequential()
    model.add(Conv2D(nb_filters, (kernel_size[0], kernel_size[1]), border_mode='valid', input_shape=input_shape, data_format="channels_first"))
    model.add(BatchNormalization(axis=1, mode=0, momentum=0.9, weights=None))
...
if __name__ == '__main__':
    ...
    model.save_weights("TrainedModels/model_CNN.h5")
