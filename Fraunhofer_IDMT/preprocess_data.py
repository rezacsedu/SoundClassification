...
def get_class_names(path="/home/karim/UrbanSound8K/AudioClassification_Deep_Learning/Fraunhofer_IDMT/Samples/"):  # class names are subdirectory names in Samples/ directory
    class_names = os.listdir(path)
    return class_names

def preprocess_dataset(inpath="/home/karim/UrbanSound8K/AudioClassification_Deep_Learning/Fraunhofer_IDMT/Samples/", outpath="Preproc/"):

    if not os.path.exists(outpath):
        os.mkdir( outpath, 755 );   # make a new directory for preproc'd files
    ...
    for idx, classname in enumerate(class_names):   # go through the subdirs
        ...
        if not os.path.exists(outpath+classname):
            os.mkdir( outpath+classname, 755 );   # make a new subdirectory for preproc class
